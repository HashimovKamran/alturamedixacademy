...
final class AlturaPageBuilderService
{
    ...
    public function bootstrap(string $language, string $pageKey): array
    {
        ...
        $draft = DB::table('aa_visual_page_revisions')
            ...
            ->latest('id')
            ->first();
        $active = $page->active_revision_id
            ? DB::table('aa_visual_page_revisions')->where('id', $page->active_revision_id)->first()
            : null;
        $source = $draft ?: $active;
        ...
        return [
            ...
            'revisions' => DB::table('aa_visual_page_revisions')
                ->where('page_id', $page->id)
                ->latest('revision_number')
                ->limit(60)
                ->get()
                ->map(fn ($row) => $this->revisionPayload($row))
                ->values()
                ->all(),
        ];
    }
    ...
    public function pageList(string $language): array
    {
        ...
        foreach (DB::table('aa_pages')->where('lang_code', $language)->get(['page_key', 'title']) as $row) {
            $keys[$this->pageKey($row->page_key)] = $row->title;
        }
        ...
        foreach ($keys as $key => $title) {
            $pages[] = $this->pagePayload($existing->get($key) ?: $this->ensurePage($language, $key, (string) $title));
        }
        foreach ($existing as $key => $page) {
            if (! array_key_exists($key, $keys)) $pages[] = $this->pagePayload($page);
        }
    ...
    public function saveDraft(string $language, string $pageKey, array $payload, ?int $actorId): array
    {
        ...
        return DB::transaction(function () use ($language, $pageKey, $document, $theme, $meta, $expected, $actorId): array {
            ...
            if ($draft && (int) $draft->editor_revision !== $expected) {
                throw new RuntimeException('Bu səhifə başqa pəncərədə dəyişdirilib. Səhifəni yeniləyin.');
            }
            if (! $draft && $expected !== 0) {
                throw new RuntimeException('Draft artıq aktual deyil. Səhifəni yeniləyin.');
            }
            ...
            return [
                'page' => $this->pagePayload(DB::table('aa_visual_pages')->where('id', $page->id)->first()),
                'draft' => $this->revisionPayload($revision),
            ];
        });
    }
    ...
    public function deletePage(string $language, string $pageKey, ?int $actorId): void
    {
        DB::transaction(function () use ($language, $pageKey, $actorId): void {
            $page = $this->ensurePage($language, $pageKey, null, true);
            if (in_array($page->page_key, ['index', '__header', '__footer'], true)) {
                throw ValidationException::withMessages(['page' => 'Bu sistem səhifəsi silinə bilməz.']);
            }
    ...
    public function publicDocument(string $language, string $pageKey, bool $preview = false): array
    {
        $page = DB::table('aa_visual_pages')
            ...
            ->where('page_key', $this->pageKey($pageKey))
            ->first();
        ...
        $revision = $preview
            ? DB::table('aa_visual_page_revisions')->where('page_id', $page->id)->where('status', 'draft')->latest('id')->first()
            : null;
    ...
    public function publicTheme(string $language, string $pageKey, bool $preview = false): array
    {
        ...
        $revision = $preview
            ...
            : null;
        if (! $revision && $page->active_revision_id) {
            $revision = DB::table('aa_visual_page_revisions')->where('id', $page->active_revision_id)->where('status', 'published')->first();
        }
    ...
    public function assets(int $page = 1, int $perPage = 36): array
    {
        ...
        return [
            'data' => $query->forPage(max(1, $page), $perPage)->get()->map(fn ($asset) => $this->assetPayload($asset))->all(),
            'meta' => ['page' => max(1, $page), 'per_page' => $perPage, 'total' => $total, 'has_more' => $page * $perPage < $total],
        ];
    }
    ...
    public function uploadAsset(UploadedFile $file, ?string $alt, ?int $actorId): array
    {
        ...
        if (! in_array($mime, ['image/jpeg', 'image/png', 'image/webp', 'image/gif'], true)) {
            throw ValidationException::withMessages(['file' => 'Yalnız JPEG, PNG, WEBP və GIF şəkilləri yüklənə bilər.']);
        }
        if ($file->getSize() > 10 * 1024 * 1024) {
            throw ValidationException::withMessages(['file' => 'Şəkil maksimum 10 MB ola bilər.']);
        }

        $dimensions = @getimagesize($file->getRealPath());
        if (! $dimensions || empty($dimensions[0]) || empty($dimensions[1])) {
            throw ValidationException::withMessages(['file' => 'Yüklənən fayl etibarlı şəkil olmalıdır.']);
        }
    ...
    {
        DB::table('aa_visual_assets')->where('id', $id)->where('is_deleted', false)->update([
            'alt_text' => $this->plain($alt, 255),
            'updated_at' => now(),
        ]);
    ...
    public function deleteAsset(int $id): void
    {
        $asset = DB::transaction(function () use ($id): object {
            ...
            if (DB::table('aa_visual_page_revision_assets')->where('asset_id', $id)->exists()) {
                throw ValidationException::withMessages(['asset' => 'Bu media səhifə revision-u tərəfindən istifadə edilir və silinə bilməz.']);
            }
            DB::table('aa_visual_assets')->where('id', $id)->update([
                'is_deleted' => true,
                'deleted_at' => now(),
                'updated_at' => now(),
            ]);
    ...
    private function ensurePage(string $language, string $pageKey, ?string $title = null, bool $lock = false): object
    {
        ...
        try {
            $id = DB::table('aa_visual_pages')->insertGetId([
                'lang_code' => $language,
                'page_key' => $key,
                'title' => $title ?: $this->pageTitle($language, $key),
                'meta_title' => null,
    ...
    {
        DB::table('aa_visual_pages')->where('id', $pageId)->update([
    ...
    private function syncAssetReferences(int $revisionId, array $document): void
    {
        ...
        if ($ids !== []) {
            ...
            if (count($found) !== count($ids)) {
                throw ValidationException::withMessages(['document' => 'Seçilən media fayllarından biri artıq mövcud deyil.']);
            }
        }

        DB::table('aa_visual_page_revision_assets')->where('revision_id', $revisionId)->delete();
        foreach ($ids as $assetId) {
            DB::table('aa_visual_page_revision_assets')->insert([
        ...
        $walk = function (array $items) use (&$walk, &$ids): void {
            foreach ($items as $node) {
                ...
                foreach ((array) ($node['settings'] ?? []) as $key => $value) {
                    if ((str_ends_with((string) $key, '_asset_id') || str_ends_with((string) $key, '_image_id') || in_array($key, ['asset_id', 'image_id'], true)) && filter_var($value, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]])) {
                        $ids[] = (int) $value;
                    }
                }
    ...
    private function revisionPayload(object $row): array
    {
        return [
            ...
            'theme_settings' => $this->decode($row->theme_settings),
    ...
    private function assetPayload(object $asset): array
    {
        return [
            ...
            'path' => $asset->path,
            'url' => Storage::disk((string) $asset->disk)->url((string) $asset->path),
    ...
    {
        $out = [];
        foreach (Arr::dot($theme) as $key => $value) {
            if (in_array($key, ['colors.primary', 'colors.accent', 'colors.surface'], true) && is_string($value) && preg_match('/^#[0-9a-fA-F]{3,8}$/', $value)) {
                $out[$key] = $value;
            }
        }
        return $out;
    ...
    private function pageKeyFromUrl(string $url): string
    {
        ...
        return isset($query['key'])
            ? $this->pageKey((string) $query['key'])
            : $this->pageKey((string) preg_replace('/\.php$/i', '', basename((string) ($parts['path'] ?? $url))));
    }
