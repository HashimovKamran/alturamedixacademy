...
return new class extends Migration {
    public function up(): void
    {
        ...
        foreach ($keys as $key) {
            ...
            foreach ($revisions->filter(fn ($row) => $this->get($row, 'lang_code') === $language && $this->get($row, 'page_key') === $pageKey)->sortBy('version') as $legacy) {
                ...
                $number++;
                $id = $this->revision($page->id, $number, 'published', $document, $this->get($legacy, 'created_by'), $this->get($legacy, 'created_at'), 'Imported legacy revision');
                $activeId ??= $id;
            }

            $publication = $publications->first(fn ($row) => $this->get($row, 'lang_code') === $language && $this->get($row, 'page_key') === $pageKey);
            if ($publication) {
            ...
            $draft = $documents->first(fn ($row) => $this->get($row, 'lang_code') === $language && $this->get($row, 'page_key') === $pageKey);
            $document = $draft ? $this->document($this->get($draft, 'document_json')) : $this->fromBlocks($blocks->filter(fn ($row) => $this->get($row, 'lang_code') === $language && $this->get($row, 'page_key') === $pageKey));
            if (! $this->empty($document)) {
                $number++;
                $this->revision($page->id, $number, 'draft', $document, $draft ? $this->get($draft, 'updated_by') : null, null, 'Imported legacy working draft', 1);
            }
            if ($activeId) DB::table('aa_visual_pages')->where('id', $page->id)->update(['active_revision_id' => $activeId, 'updated_at' => now()]);
        }

        foreach (['aa_legacy_page_builder_documents','aa_legacy_page_builder_blocks','aa_legacy_page_publications','aa_legacy_page_revisions','aa_legacy_block_patterns'] as $table) {
    ...
    {
        // Legacy tables are imported into immutable visual revisions and are not recreated on rollback.
    }
    ...
    private function page(string $language, string $pageKey): object
    {
        ...
        $id = DB::table('aa_visual_pages')->insertGetId(['lang_code' => $language, 'page_key' => $pageKey, 'title' => $title ?: ucfirst(str_replace(['_','-'], ' ', $pageKey)), 'is_archived' => false, 'is_deleted' => false, 'lock_version' => 1, 'created_at' => now(), 'updated_at' => now()]);
    ...
    {
        return DB::table('aa_visual_page_revisions')->insertGetId(['page_id' => $pageId, 'revision_number' => $number, 'status' => $status, 'editor_revision' => $editorRevision, 'document_json' => json_encode($document, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), 'theme_settings' => json_encode([]), 'change_note' => $note, 'created_by' => $actor, 'published_by' => $status === 'published' ? $actor : null, 'published_at' => $status === 'published' ? ($publishedAt ?: now()) : null, 'created_at' => now(), 'updated_at' => now()]);
    }
    ...
    private function convertMap(array $items, array $order): array
    {
        ...
        foreach (($order ?: array_keys($items)) as $id) {
            ...
            $sections[(string) $id] = ['type' => ($row['type'] ?? 'rich_text') === 'text' ? 'rich_text' : ($row['type'] ?? 'rich_text'), '_name' => null, 'disabled' => (bool) ($row['disabled'] ?? false), 'slot_key' => $row['slot_key'] ?? 'default', 'settings' => $settings, 'blocks' => $children['sections'], 'order' => $children['order']];
        }
    ...
    private function fromRows(array $rows): array
    {
        ...
        $build = function (string $parent = '__root') use (&$build, $byParent): array {
            ...
            foreach (($byParent->get($parent, collect()))->sortBy('sort_order') as $row) {
                $uuid = (string) ($row['block_uuid'] ?? ('legacy_'.$row['id']));
                $content = $this->decode($row['content_json'] ?? null);
                if ($content === []) $content = array_filter(['title' => $row['title'] ?? null, 'eyebrow' => $row['subtitle'] ?? null, 'html' => $row['body'] ?? null, 'button_text' => $row['button_text'] ?? null, 'button_url' => $row['button_url'] ?? null], fn ($value) => $value !== null && $value !== '');
                [$children, $childOrder] = $build($uuid);
                $nodes[$uuid] = ['type' => ($row['block_type'] ?? 'rich_text') === 'text' ? 'rich_text' : ($row['block_type'] ?? 'rich_text'), '_name' => null, 'disabled' => ! (bool) ($row['is_active'] ?? true), 'slot_key' => $row['slot_key'] ?? 'default', 'settings' => $content, 'blocks' => $children, 'order' => $childOrder];
                $order[] = $uuid;
            }
            return [$nodes, $order];
        };
