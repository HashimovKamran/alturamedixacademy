...
<header class="site-header" id="siteHeader" data-block-uuid="{{ $block->block_uuid }}">
    <div class="header-top"><div class="container header-top-inner">
        ...
        <div class="header-tools">
            @if($enabled('show_languages'))<nav class="language-switch" aria-label="Language">@foreach($languages->filter(fn($language)=>$language->code!==$lang) as $language)<a href="{{ app(\App\Services\Site\SeoService::class)->alternate(request(),$language->code) }}">{{ strtoupper($language->code) }}</a>@unless($loop->last)<span>|</span>@endunless @endforeach</nav>@endif
    ...
    <div class="header-nav"><div class="container nav-inner">
        ...
        <div class="nav-actions" id="navActions">
            ...
            @if($enabled('show_search'))<button class="search-btn" type="button" aria-label="{{ $ui['search'] }}" data-site-search-open><i class="fa-solid fa-magnifying-glass"></i></button>@endif
